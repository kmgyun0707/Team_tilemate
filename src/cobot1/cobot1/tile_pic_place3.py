...
# ----------------------------
USE_GRIPPER = True  # ★ 그리퍼 사용 (실제 로봇에서 사용)
IP = "192.168.1.1"
PORT = 502
...
def set_gripper(width_m: float, force_n: float = 40.0):
    ...
    - width_m: m 단위 목표 폭
    - force_n: N 단위 목표 힘(여기서는 0.1N 단위로 변환한다고 가정)

    순서:
    1) force write
    2) width write
    3) start write
    4) status busy bit가 0 될 때까지 polling
    """
    if not USE_GRIPPER:
    ...
    try:
        ...
        # 실제 장비는 오프셋/오차가 있으니 이 비율은 튜닝 포인트
