#!/usr/bin/env python3
# tilemate_main/new_tile_motion_node.py
...
import DR_init
from robot_config import RobotConfig, GripperConfig
from onrobot import RG

class NewTileMotionNode(Node):
    def __init__(self, robot_cfg, boot_node):
        super().__init__("new_tile_motion_node", namespace=robot_cfg.robot_id)
        self.robot_cfg = robot_cfg
        self._boot_node = boot_node
        self.gripper = RG(GripperConfig.GRIPPER_NAME,GripperConfig.TOOLCHARGER_IP,GripperConfig.TOOLCHARGER_PORT)
...
def main(args=None):
    ...
    boot = rclpy.create_node("dsr_boot_tile_motion_job", namespace=robot_cfg.robot_id)
    ...
    node = NewTileMotionNode(robot_cfg, boot)
